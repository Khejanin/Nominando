...
namespace SwipeMenu
{
	...
	public class Menu : MonoBehaviour
    {
        ...
        private readonly float _centreOffset = 1.0f;
        private float _currentMenuPosition;
        private float _maxMenuPosition;
        private SwipeHandler _swipeHandler;

        /// <summary>
        ///     The distance between menus. Distance between menus must be divisible by 0.5f. This is clamped in the Awake
        ///     function.
        /// </summary>
        ...
        public static Menu instance
        {
            get
            {
                if (!_instance) _instance = FindObjectOfType<Menu>();
        ...
        private void Awake()
        {
            ...
            if (GetComponent<TouchHandler>() == null) gameObject.AddComponent<TouchHandler>();

            _swipeHandler = GetComponent<SwipeHandler>();

            if (_swipeHandler == null) _swipeHandler = gameObject.AddComponent<SwipeHandler>();
        }
        ...
        {
            var currentIndex = GetClosestMenuItemIndex();
        ...
        {
            var offset = CalcPosXInverse(item.transform.position.x);
        ...
        /// <summary>
        ///     Provides a direct/constant movement by the specified amount. Not animated. Used for swipes that are not classed as
        ///     flicks.
        /// </summary>
        /// <param name="amount">Amount.</param>
        ...
        {
            foreach (var menu in menuItems) menu.gameObject.SetActive(false);
        }
        ...
        {
            foreach (var menu in menuItems) menu.gameObject.SetActive(true);
        }
        ...
        private void UpdateMenuItemsPositionInWorldSpace()
        {
            for (var i = 0; i < menuItems.Length; i++)
            {
                var offsetx = distanceBetweenMenus * (i + 1) - _currentMenuPosition;
                var posx = CalculateMenuItemXPosition(offsetx);
                var posz = CalculateMenuItemZPosition(offsetx);
                var pos = new Vector3(posx, 0, posz);
                menuItems[i].transform.position = pos;
                var euler = new Vector3(0, CalculateMenuItemRotation(offsetx), 0);
                menuItems[i].transform.eulerAngles = euler;
            }
        }

        /// <summary>
        ///     Updates the _current menu position.
        /// </summary>
