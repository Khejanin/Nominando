...
namespace SwipeMenu
{
	...
	public class SwipeHandler : MonoBehaviour
    {
        ...
        public bool isSwiping => SW || length != 0;
        ...
        private void HandleMobileSwipe()
        {
            if (Input.touchCount > 0)
            {
                if (Input.GetTouch(0).phase == TouchPhase.Began)
                {
                    ...
                    length = 0;
                    SW = false;
                    var touchDeltaPosition = Input.GetTouch(0).position;
                ...
                if (Input.GetTouch(0).phase == TouchPhase.Moved)
                {
                    ...
                    var touchDeltaPosition = Input.GetTouch(0).position;
                    var pos = new Vector3(touchDeltaPosition.x, 0, touchDeltaPosition.y);
                ...
                if (Input.GetTouch(0).phase == TouchPhase.Canceled) SW = false;

                if (Input.GetTouch(0).phase == TouchPhase.Stationary) SW = false;
                ...
                if (Input.GetTouch(0).phase == TouchPhase.Ended)
                {
                    if (SW && handleFlicks)
                    {
                        ...
                        length = finalPosition.x < 0
                            ? -(finalPosition.magnitude * Time.deltaTime)
                            : finalPosition.magnitude * Time.deltaTime;
                        ...
                        if (handleFlicks && Mathf.Abs(force) > requiredForceForFlick) Menu.instance.Inertia(-length);
                    }

                    if (lockToClosest) Menu.instance.LockToClosest();
                }
            }
        }
        ...
        private void HandleMouseSwipe()
        {
            ...
            if (handleSwipes && Input.GetMouseButton(0) && mouseMove != 0) Menu.instance.Constant(-(mouseMove * .1f));
        }
    }
}